...
def _ffmpeg_before_options(base_before: str, info: Dict[str, Any], webpage_url: str) -> str:
    ...
    if user_agent:
        # Quote UA because it typically contains spaces/parentheses
...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    YTDL_OPTIONS: Dict[str, Any] = {
        ...
        # Optional: try alternative YouTube clients that can be less fragile on some networks.
        # This mirrors CLI: --extractor-args "youtube:player_client=android,web"
        "extractor_args": {"youtube": {"player_client": ["android", "web"]}},
    }

    FFMPEG_OPTIONS: Dict[str, str] = {
        "before_options": "-reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5 -rw_timeout 15000000",
        # Don't wrap the filtergraph in quotes; some environments pass quotes literally.
        ...
        for url in webpage_urls:
            ...
            if "entries" in info and info["entries"]:
                for entry in info["entries"]:
                    if not entry:
                        continue
                    before = _ffmpeg_before_options(cls.FFMPEG_OPTIONS.get("before_options", ""), entry, entry.get("webpage_url") or url)
                ...
                before = _ffmpeg_before_options(cls.FFMPEG_OPTIONS.get("before_options", ""), info, info.get("webpage_url") or url)
                ffmpeg_opts = dict(cls.FFMPEG_OPTIONS)
                ffmpeg_opts["before_options"] = before
    ...
    async def regather_stream(
        ...
        *,
        ...
        refreshed = cls(ctx, discord.FFmpegPCMAudio(info["url"], **ffmpeg_opts), data=info, volume=source.volume)
        return refreshed
